...
> A–D came from the original audit. **E and F were found while writing the Phase-1 port** (2026-08-02) and are at least as important; both are in `docs/REFERENCE_NOTES.md` with evidence. G and H followed from the Euclidean kernel work, and a **window-boundary off-by-one** in the exponential histogram came out of a tier-1 test.
>
> **The count is seven defects, and it is reached the same way in every document:** 2 + 3 + 2.
>
> - **Two are in the published algorithm itself** (Algorithm 2, §4.1), active at the `k = 1` the paper uses for every experiment: **A** (every cell drops its first arrival) and **F** (cold cells never expire, so density is frozen).
> - **Three are code-only defects in the reference, latent at `k = 1`:** **E** (angular cell code encodes only Hamming weight), **G** (Euclidean cell code sums the `k` hashes), **H** (L2 ground truth omits `** k`).
> - **Two are sliding-window semantics defects in the reference's exponential histogram**, active at any `k`: **B** (single-step expiry under-evicts) and the **window-boundary off-by-one** (`<` keeps `N+1` elements where the paper's Problem 1.2 defines `N`).
>
> **C and D are excluded because they are not defects.** C (mean vs median-of-means) is the reference being *correct* — the paper does specify the mean for SW-AKDE. D is our validation strategy, not a flaw in the source material. **Do not write "eight" anywhere.** Any count reaching eight is double-counting, most often by treating Finding B's multi-bucket eviction as an item separate from Finding B itself — it is the *same* defect, and §5's table groups it accordingly. Canonical enumeration, with per-finding evidence: `docs/PROJECT_RECORD.md` §4.
